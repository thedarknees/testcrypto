...
CHAT_ID = "-1004378724412"  # id канала/группы, куда создаём ссылки
ADMIN_IDS = {1035443294, 222222222}  # user_id тех, кому разрешено создавать ссылки; пусто = разрешено всем
# ====================
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = conn.execute(
        ...
        FROM links l
        LEFT JOIN joins j ON j.link_name = l.name
        GROUP BY l.name
    ...
    for r in rows:
        ...
        total += joined
        lines.append(f"• {r['name']}: перешло {joined}, осталось в чате {active}")
